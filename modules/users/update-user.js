module.exports = function({connection, req}){
    return new Promise((resolve, reject)=>{
        const { id } = req.params;
        const {usuario, pswd, nombres, apellidos, email, id_rol, session_time, usuario_creo, usuario_modifico, usuario_elimino, fecha_creo, fecha_modifico, fecha_elimino} = req.body;
        ...
        usuario_creo = ?, usuario_modifico = ?, usuario_elimino = ?, fecha_creo = ?, fecha_modifico = ?, fecha_elimino = ? WHERE id = ?`;
        console.log(usuario, pswd, nombres, apellidos, email, id_rol, session_time);
        connection.query(query, [usuario, pswd, nombres, apellidos, email, id_rol, session_time, 1, usuario_creo, usuario_modifico, usuario_elimino, fecha_creo, fecha_modifico, fecha_elimino, id], (ex,{rows}) => {
            if (ex) {
              console.log(ex);
