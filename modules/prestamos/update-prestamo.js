module.exports = function({connection, req}){
  return new Promise((resolve, reject)=>{
    const { id } = req.params;
    const {nombres, apellidos, email, dni, id_objeto, motivo, firma, tiempo_prestamo, hora_prestamo, hora_devolucion, usuario_creo, usuario_modifico} = req.body;
    ...
    const query = `UPDATE tb_prestamos SET nombres = ?, apellidos = ?, email = ?, dni = ?, id_objeto = ?, motivo = ?, firma = ?,
      tiempo_prestamo = ?, usuario_modifico = ? WHERE id = ?`;
    ...
    connection.query(query, [nombres, apellidos, email, dni, id_objeto, motivo, firma, tiempo_prestamo, usuario_modifico, id], (ex,{rows}) => {
        if (ex) {
          console.log(ex);
