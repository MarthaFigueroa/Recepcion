module.exports = function({connection, req}){
    return new Promise((resolve, reject)=>{
      const { id } = req.params;
      const {id_objeto, motivo, cantidad, reparado, usuario_creo} = req.body;
      
      if(reparado === 0){

        const query = `UPDATE tb_defectuosos SET id_objeto = ?, motivo = ?, cantidad = ?, reparado = ?, usuario_creo = ? WHERE id = ?`;
        // console.log(categoria, usuario_creo);
        connection.query(query, [id_objeto, motivo, cantidad, reparado, usuario_creo, id], (ex,{rows}) => {
            if (ex) {
              console.log(ex);
