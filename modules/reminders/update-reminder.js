module.exports = function({connection, req}){
    return new Promise((resolve, reject)=>{
        const { id } = req.params;
        const {titulo, descripcion, usuario_modifico, completado} = req.body;
        ...
        if(completado === 1){
          const query = `UPDATE tb_recordatorios SET completado = ? WHERE id = ?`;
          // console.log(categoria, usuario_creo);
          connection.query(query, [completado, id], (ex,{rows}) => {
              if (ex) {
                console.log(ex);
                reject({msg: "Error en consulta en DB"})
              } else {
                console.log("Rows: ",rows);
                resolve({status: `Recordatorio con id: ${id} ha sido completado exitosamente!`})
              }
          })
        }else {
